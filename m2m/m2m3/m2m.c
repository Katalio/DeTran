...
#define tcp_down_recv_size 	64100
#define udp_down_def_size 	2048	
#define udp_down_recv_size 	2100
...
static int tcp_socket_recv(char *pdu_buf, int pdu_len)
{
	...
	tmp_buf = (char *)malloc(tcp_down_recv_size);
	if(tmp_buf)
	{
		while(1)
		{
			...
			if (recv_len > tcp_down_def_size)	//if(ret == 0)
			{
				break;
			}
...
static int udp_download_process(FILE *file_fd)
{
	...
	if(file_fd != NULL)
	{
		while((tcp_socket = connect_m2m_tcp_server(0, 0, g_m2m_server_ip, m2m_config.svr_port)) < 0)
		{
			...
			if(retry >= 5)
			{
				...
				if(udp_download_process(file_fd))	//if tcp connect fail, use udp to download
				{
					fclose(file_fd);
					return 1;
				}
		...
		recv_count = 0;
		recv_buf = (char *)malloc(tcp_down_recv_size);
		if(recv_buf)
		{
			while (recv_count < total_count)
			{
