...
#define TCP_CONNECT_FAIL_TIMES 5
enum protocol
{
    TCP = 0,
    UDP
...
M2M_PROTOCOL_DOWNLOAD_PARAM m2m_dp;
M2M_PROTOCOL_DOWNLOAD_FILE m2m_df;
...
static int read_tcp_packet( int fd, char* buf, unsigned int len)
{
    ...
    while(1)
    {
        if(wait_sock(fd, 5, 0) > 0)
        {
            tcp_pack_len += recv(fd,buf + tcp_pack_len,len - tcp_pack_len,0);
            if(len == tcp_pack_len)
            {
                ret = tcp_pack_len;
                break;
            }
            else if(tcp_pack_len > len)
            {
...
int udp_socket_send(char *pdu_buf, int pdu_len)
{
	...
	syslog(LOG_NOTICE, "SEND LENGTH = %d", pdu_len);
...
//send data bytes
static unsigned long up_trafic_calc(void)
{
    FILE *f;
    char buf[256];
    char *ifname;
    ...
    while (fgets(buf, sizeof(buf), f))
    {
        ...
        *p = 0;
        ...
        {
            // <rx bytes, packets, errors, dropped, fifo errors, frame errors, compressed, multicast><tx ...>
            if (sscanf(p + 1, "%*u%*u%*u%*u%*u%*u%*u%*u%lu", &counter) == 1)
            {
            	counter = counter / 1024;
                pre_trafic = nvram_get_int("m2m_up_trafic");
                if(counter < pre_trafic)
                {
                    trafic = counter;
                }
                else
                {
                    trafic = counter - pre_trafic;
                }
                if (1)
                {
                    trafic_flag = 1;
    ...
    fclose(f);
    syslog(LOG_NOTICE, "up trafic: %lu:pre=%lu:new=%lu", counter, pre_trafic, trafic);
...
int download_file_report(unsigned char status, unsigned int req_file_offset, unsigned int packetid)
{
    unsigned int downloadNum;
    char respbuf[128] = {0};
    M2M_PROTOCOL_HDR *m2m_res = respbuf;
    int n = 0;
    ...
    if(socket_fd > 0)
    {
        ...
        m2m_res->packet_id = packetid;
        m2m_res->version = htons(M2M_VERSION);
        m2m_res->safe_flag = 0;
        m2m_res->data_type = 0;
        memcpy(m2m_res->product_id, product_id, sizeof(m2m_res->product_id));
        m2m_res->len = htons(sizeof(M2M_PROTOCOL_HDR) + 5);

        *(respbuf + sizeof(M2M_PROTOCOL_HDR)) = status;//download failed or success flag
        downloadNum = htonl(req_file_offset);
        memcpy(respbuf + sizeof(M2M_PROTOCOL_HDR) + 1, &downloadNum, 4);
        n = udp_socket_send(respbuf, sizeof(M2M_PROTOCOL_HDR) + 1 + 4);
    }
}
...
int m2m_send_cmd(unsigned short cmd)
{
    ...
    if(socket_fd >= 0)
    {
        ...
        else if(cmd == REPORT_STATUS)
        {
            if(nvram_get_int("cell_cops") == 7)
            {
                if(strlen(nvram_safe_get("celle_lac")))
                {
                    tlv = (M2M_PROTOCOL_TLV *)(hb_buf + hb_len);
                    str_len = strlen(nvram_safe_get("celle_lac"));
                    tlv->tlv_tag = htons(BASE_STATION1_LAC_TAG);
                ...
                if(strlen(nvram_safe_get("celle_cid")))
                {
                    tlv = (M2M_PROTOCOL_TLV *)(hb_buf + hb_len);
                    str_len = strlen(nvram_safe_get("celle_cid"));
                    tlv->tlv_tag = htons(BASE_STATION1_CELLID_TAG);
            ...
            {
                if(strlen(nvram_safe_get("cellg_lac")))
                {
                    tlv = (M2M_PROTOCOL_TLV *)(hb_buf + hb_len);
                    str_len = strlen(nvram_safe_get("cellg_lac"));
                    tlv->tlv_tag = htons(BASE_STATION1_LAC_TAG);
                ...
                if(strlen(nvram_safe_get("cellg_cid")))
                {
                    tlv = (M2M_PROTOCOL_TLV *)(hb_buf + hb_len);
                    str_len = strlen(nvram_safe_get("cellg_cid"));
                    tlv->tlv_tag = htons(BASE_STATION1_CELLID_TAG);
...
{
    struct stat st;
    int offset = 0, file_size, read_len, ret = -1;
    FILE *fp;
    char md5[33];
...
void tcp_download_process(void *arg)
{
    int sock_fd = -1;
    int count = 0;
    unsigned int offset = 0;
    unsigned int packetid=(int)arg;

    while(1)
    {
        sock_fd = connect_tcp_host(g_m2m_server_ip, m2m_config.svr_port, 3);
        if (sock_fd < 0)
        {
            syslog(LOG_ERR,  "TCP Failed to connect(%s:%s)", g_m2m_server_ip, m2m_config.svr_port );
            count++;
            if(count >= TCP_CONNECT_FAIL_TIMES)
            {
                break;
            }
            sleep(60);
        ...
        {
            offset = req_file_thread(packetid, TCP, sock_fd);
            close(sock_fd);
            if(req_file_finish_flag != 1)
            {
                download_file_report(1, offset, packetid);//download failed
                syslog(LOG_INFO,"[%x]------->Download file fialed",DOWNLOAD_INFO_ACK);
            }
            else
            {
                download_file_report(0, offset, packetid);//download success
                req_file_finish_flag = 0;
                do_something_after_recv_file(ntohl(packetid));
                syslog(LOG_INFO,"[%x]------->Download file success",DOWNLOAD_INFO_ACK);
                logout_reboot();
            }
            return;
        }
    }
    offset = req_file_thread(packetid, UDP, socket_fd);
    if(req_file_finish_flag != 1)
    {
        download_file_report(1, offset, packetid);//download failed
        syslog(LOG_INFO,"[%x]------->Download file fialed",DOWNLOAD_INFO_ACK);
...
void do_something_after_recv_file(int packetid)
{
    int pid = -1;
    int n;
    ...
    if(nvram_match("type","hk_camera"))
    {
        if((!g_exdev_running))
        {
            ...
            if (pthread_create(&exdev_handle_id, NULL, (void *)exdev_handle_thread, packetid) != 0)
            {
    ...
    if(strstr(m2m_df.file_name,".patch"))
    {
    ...
    else if(strstr(m2m_df.file_name,".trx"))
    {
    ...
    else if(strstr(m2m_df.file_name,".cfg"))
    {
        static char *args[] = {"nvram", "restore", filename, NULL};
        _eval(args, ">/tmp/tmp.cfg.msg", 0, NULL);
    }
    else if(strstr(m2m_df.file_name,".bin"))
    {
        serial_upgrade();
    }
    unlink(m2m_df.file_name);
    AD_UPGRADE_flag = 0;
}
void logout_reboot(void)
{
...
static int process_packet(char* pdu_ptr, int pdu_len)
{
    ...
    char cmdResult;
    unsigned int req_file_offset, downloadNum;//下载进度
    char *p;
    int len;
...
#endif
                break;
            case DOWNLOAD_INFO://文件上传与下载
                syslog(LOG_NOTICE, "M2M Command(%02x) DOWNLOAD_INFO!!!", ntohs(m2m_req->cmd_id));
                memcpy(&m2m_dp, pdu_ptr + sizeof(M2M_PROTOCOL_HDR), sizeof(M2M_PROTOCOL_DOWNLOAD_PARAM));
                memcpy(&m2m_df, pdu_ptr + sizeof(M2M_PROTOCOL_HDR) + sizeof(M2M_PROTOCOL_DOWNLOAD_PARAM), sizeof(M2M_PROTOCOL_DOWNLOAD_FILE));
                if(*(pdu_ptr + sizeof(M2M_PROTOCOL_HDR)) == 0x2)//tcp
                {
                    pthread_t tcp_download;
                    if(!pthread_create(&tcp_download, NULL, (void *)tcp_download_process, m2m_req->packet_id))
                    {
                        syslog(LOG_INFO,"Create tcp download thread error!");
                    }
                    else
                    {
                        pthread_detach(tcp_download);
                    }
                }
                else
                {
                    req_file_offset = req_file_thread(m2m_req->packet_id, UDP, socket_fd);
                    if(req_file_finish_flag != 1)
                    {
                        download_file_report(1, req_file_offset, m2m_req->packet_id);//download failed
                        syslog(LOG_INFO,"[%x]------->Download file fialed",DOWNLOAD_INFO_ACK);
                    }
                    else
                    {
                        download_file_report(0, req_file_offset, m2m_req->packet_id);//download success
                        req_file_finish_flag = 0;
                        do_something_after_recv_file(ntohl(m2m_req->packet_id));
                        syslog(LOG_INFO,"[%x]------->Download file success",DOWNLOAD_INFO_ACK);
                        logout_reboot();
                    }
                }
...
void heartbeat_report_thread(void *param)
{
    ...
    while(fd  >= 0)
    {
        ...
        if(r_login_ack)
        {
            ...
            *(hb_buf + sizeof(M2M_PROTOCOL_HDR)+1+4+1) = nvram_get_int("sim_flag") == 2 ? 2 : 1;//nvram_get_int("sim_flag")
            *(unsigned int*)(hb_buf + sizeof(M2M_PROTOCOL_HDR)+1+4+1+1) = htonl(up_trafic_calc());
            hb_len = sizeof(M2M_PROTOCOL_HDR)+1+4+1+1+4;
...
//return the length of the download file
int req_file_thread(int taskId, int protocol, int socketFd)
{
    M2M_PROTOCOL_HDR *file_req_head, *m2m_req;
    M2M_PROTOCOL_FILE_REQ *file_req_data;
    unsigned int req_file_len, req_file_offset = 0;
    char filereqBuf[BUF_LEN] = {0};
    char pdubuf[M2M_RES_PDU_BUF] = {0};
    int i, n;
    unsigned int sendTime = 0;
    unsigned int fileCount = 0;
    FILE *fp;
    unsigned int pdu_len = 0;
    unsigned int need_tcp_read_len = 0;

    req_file_len = ntohl(m2m_df.file_len);

    snprintf(filename, sizeof(filename), "%s%s", FTP_FILE_TMP_PATH, m2m_df.file_name);
    while((fp = fopen(filename,"wb")) == NULL)
    {
        ...
        {
            fileCount = 0;
            return req_file_offset;
        }
    }
    syslog(LOG_INFO,"file_len:%d, file_name:%s, file_id:%d, file_md5:%s", ntohl(m2m_df.file_len), m2m_df.file_name, ntohl(m2m_df.file_id), m2m_df.file_md5);
    //recvieve file and save
    while(((req_file_len - req_file_offset) > 0) && (socketFd > 0))
    {
        memset(filereqBuf, 0, sizeof(filereqBuf));
        file_req_head = (M2M_PROTOCOL_HDR*)filereqBuf;
        file_req_head->cmd_id = htons(FILE_REQ);
        file_req_head->data_type = 0;
        file_req_head->packet_id = htonl(packet_id++);
        memcpy(file_req_head->product_id, product_id, sizeof(file_req_head->product_id));
        file_req_head->safe_flag = 0;
        file_req_head->version = htons(M2M_VERSION);
        file_req_head->len = htons(sizeof(M2M_PROTOCOL_HDR) + sizeof(M2M_PROTOCOL_FILE_REQ));

        file_req_data = (M2M_PROTOCOL_FILE_REQ *)(filereqBuf + sizeof(M2M_PROTOCOL_HDR));
        file_req_data->file_id = m2m_df.file_id;
        file_req_data->offset = htonl(req_file_offset);

        pdu_len = protocol ? UDP_UPLOAD_DOWNLOAD_LEN : TCP_UPLOAD_DOWNLOAD_LEN;
        if((req_file_len - req_file_offset) >= pdu_len)
        {
            file_req_data->req_len = htonl(pdu_len);
            need_tcp_read_len = pdu_len + sizeof(M2M_PROTOCOL_HDR) + 1;
        }
        else
        {
            file_req_data->req_len = htonl(req_file_len - req_file_offset);
            need_tcp_read_len = req_file_len - req_file_offset + sizeof(M2M_PROTOCOL_HDR) + 1;
        }


        file_req_data->task_id = taskId;
        if(protocol == TCP)
        {
            if(send(socketFd, filereqBuf, sizeof(M2M_PROTOCOL_HDR) + sizeof(M2M_PROTOCOL_FILE_REQ), 0) < 0)
            {
                fclose(fp);
                unlink(filename);
                return req_file_offset;
        ...
        {
            udp_socket_send(filereqBuf, sizeof(M2M_PROTOCOL_HDR) + sizeof(M2M_PROTOCOL_FILE_REQ));
        }
        ...
        while(socketFd > 0)
        {
            ...
            if(protocol == TCP)
            {
                n = read_tcp_packet(socketFd, pdubuf, need_tcp_read_len);
                syslog(LOG_INFO,"real read :%d, need to read:%d", n, need_tcp_read_len);
            }
            else
            {
                n = udp_socket_recv(pdubuf, sizeof(pdubuf));
            ...
            if(n > 0)//Recv pdu
            {
                ...
                if((n > sizeof(M2M_PROTOCOL_HDR)))
                {
                    //syslog(LOG_INFO,"--->Recv len: %d, cmd :%x", n, ntohs(m2m_req->cmd_id));
                    m2m_req = (M2M_PROTOCOL_HDR *)pdubuf;

                    if(ntohs(m2m_req->cmd_id) == FILE_REQ_ACK)
                    {
                        fwrite(pdubuf + sizeof(M2M_PROTOCOL_HDR) + 1, 1, n - sizeof(M2M_PROTOCOL_HDR) - 1, fp);
                        req_file_offset += (n - sizeof(M2M_PROTOCOL_HDR) - 1);
                        syslog(LOG_INFO,"----->Offset:%d", req_file_offset);
                        sendTime = 0;
                        break;
                    }
                    else if(ntohs(m2m_req->cmd_id) == M2M_HEARTBEAT_ACK)
                    ...
                    {
                        syslog(LOG_INFO,"Recv %x cmd, continue!",ntohs(m2m_req->cmd_id));
                        continue;
                    }
                }
                else
                {
                    syslog(LOG_ERR, "iRcv: %d != m2mHDR: %d", n, sizeof(M2M_PROTOCOL_HDR));
                    fclose(fp);
                    unlink(filename);
                    return req_file_offset;
                }
            }
            else//recv timeout or other error
            {
                sendTime++;
                if(sendTime >= 6)//download failed
                {
                    syslog(LOG_INFO,"Download data over 3 times,error!");
                    fclose(fp);
                    unlink(filename);
                    return req_file_offset;
                }
    ...
    fclose(fp);
    //check the md5
    if(check_md5(filename,m2m_df.file_md5) == 1)
    {
        req_file_finish_flag = 1;
        syslog(LOG_INFO,"Recv and check file ok!");
    }
    return req_file_offset;
...
int encode_rtu_pub_pack(unsigned char *buf, int buf_len)
{
    ...
    while ((b = strsep(&nvp, ">")) != NULL)
    {
        char *cmd, *regAddr, *data_type, *sigid, *alarm_flag, *desc;
        char *regAddr_value = NULL, *val_value = NULL;
        int len;

        n = vstrsep(b, "<", &cmd, &regAddr, &data_type, &sigid, &alarm_flag, &desc);
        if (n < 6)
        {
            continue ;
        }
		...
		if(atoi(alarm_flag) == 0) 	//normal data
		{
		    ...
		    tlv = (M2M_PROTOCOL_TLV *)(buf + hb_len);
...
int encode_rtu_pub_alarm_pack(unsigned char *buf, int buf_len)
{
    unsigned short hb_len = 0;
    M2M_PROTOCOL_HDR* hb = (M2M_PROTOCOL_HDR*)buf;
    M2M_PROTOCOL_TLV *tlv;
    ...
    char *nv, *nvp, *b;
    int n, valueType;
    char flag_nv_name[32] = {0};
	char clean_flag[64] = {0};
    char addr_nv_name[32] = {0};
    char val_nv_name[32] = {0};
    char status_nv_name[32] = {0};
    int val_i;
    float val_f;
    char outBuf[12] = {0};
    ...
    {
        char *cmd, *regAddr, *data_type, *sigid, *alarm_flag, *desc;
        char *val_value = NULL, *regAddr_value = NULL;
        int len;
        ...
        snprintf(flag_nv_name, sizeof(flag_nv_name), "alarmflag_%s", sigid);
		if(nvram_match(flag_nv_name, "1"))
		{
        	snprintf(addr_nv_name, sizeof(addr_nv_name), "alarmAddr_%s", sigid);
        	snprintf(val_nv_name, sizeof(val_nv_name), "alarmval_%s", sigid);
        	snprintf(status_nv_name, sizeof(status_nv_name), "alarmstate_%s", sigid);
		
        	regAddr_value = nvram_safe_get(addr_nv_name);
        	val_value = nvram_safe_get(val_nv_name);
			status = nvram_get_int(status_nv_name);
			
			syslog(LOG_NOTICE, "alarm regAddr: %s, alarm value: %s, alarm state: %d", regAddr_value, val_value, status);
        	...
        	dtlv->slave_id = 0x01;
        	dtlv->regaddr = htons(startAddr);
        	tlv->tlv_value[sizeof(RTU_PUBLISH_DATA)] = status;
        	switch(valueType)
        	{
        	    case 1://float
        	        val_f = atof(val_value);
        	        ...
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 1] = outBuf[0];
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 2] = outBuf[1];
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 3] = outBuf[2];
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 4] = outBuf[3];
        	        len = 4;
        	        break;
        	    case 2:
        	    case 3:
        	    case 4:
        	        val_i = atoi(val_value);
        	        memcpy(outBuf, &val_i, 4);
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 1] = outBuf[3];
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 2] = outBuf[2];
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 3] = outBuf[1];
        	        tlv->tlv_value[sizeof(RTU_PUBLISH_DATA) + 4] = outBuf[0];
        	        len = 4;
        	        break;
        	    default:
        	        break;
        	...
        	tlv->tlv_len = htons(sizeof(RTU_PUBLISH_DATA) + 1 + len);
        	hb_len += len + 9;

			snprintf(clean_flag, sizeof(clean_flag), "nvram unset %s=", flag_nv_name);
			system(clean_flag);
		}
    }
...
void *rtu_pub_thread_routine(void *arg)
{
    ...
    while(socket_fd < 0)
    {
      syslog(LOG_INFO,"Wait for connect to server!");
      sleep(1);
    }
    while (1)
    {        
        ...
        if(pkt_length > 28)
        {
      		syslog(LOG_NOTICE, "Normal Publish:");
            udp_socket_send(buf,pkt_length);
        }        
        sleep(pub_interval);
    }
...
void *rtu_pub_alarm_thread_routine(void *arg)
{
    unsigned char buf[2048] = {0};
    int pkt_length;
    int ret;
    ...
    {        
        memset(buf, 0, sizeof(buf));
        pkt_length = encode_rtu_pub_alarm_pack(buf, sizeof(buf));
        if(pkt_length > 28)
        {
        	syslog(LOG_NOTICE, "Alarm Publish:");
            udp_socket_send(buf,pkt_length);
        }        
    }
...
int m2m_main(int argc, char *argv[])
{
    ...
    unsigned long ip = 0;
    struct timeval  tv;
    pthread_t heart_beat_id,external_id,report_url_id,report_status_alone_id, lora_id, rtu_pub_id, rtu_pub_alarm_id;
    pthread_t sf_box;
    char *p;
    int pl = 0;
    ...
    pl = strlen(p) > 15 ? strlen(p) - 15 : 0; //取sn后15位
    snprintf(product_id, sizeof(product_id) - 1, "%s", p + pl);
    ...
    {
        syslog(LOG_ERR, "!!M2M Failed to Create Publish Thread");
    }
    else
    {
    ...
    if (pthread_create(&rtu_pub_alarm_id, NULL, (void *)rtu_pub_alarm_thread_routine, NULL) != 0)
    {
        syslog(LOG_ERR, "!!M2M Failed to Create Publish Alarm Thread");
    }
    else
    {
        pthread_detach(rtu_pub_alarm_id);
        syslog(LOG_NOTICE, "M2M rtu_pub_alarm_id Thread %d", rtu_pub_alarm_id);
    }
