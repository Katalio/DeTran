...
#define down_def_size 	4096
#define tcp_down_recv_size 	4500
...
static int tcp_socket_recv(char *pdu_buf, int pdu_len)
{
    int recv_len = 0;
	int ret = 0;
	char tmp_buf[tcp_down_recv_size];

	if(wait_sock(tcp_socket, 2, 0) < 0)
	{
		syslog(LOG_NOTICE, "DOWNLOAD TCP Recv timeout");
		return -1;
	}

	while(1)
	{
		memset(tmp_buf, 0, tcp_down_recv_size);
		ret = recv(tcp_socket, tmp_buf, pdu_len, MSG_DONTWAIT);
		if (recv_len > down_def_size)
			break;
		
		syslog(LOG_NOTICE, "%s>>>Recv Len:%d", __FUNCTION__, ret);
		
		memcpy(pdu_buf + recv_len, tmp_buf, ret);
		recv_len += ret;
	}
...
{	
	...
	total_count = ntohl(g_down_info->filelist[0].size) / down_def_size;
	if((last_size = ntohl(g_down_info->filelist[0].size) % down_def_size) != 0)
		total_count ++;
	else
		last_size = down_def_size;

	syslog(LOG_NOTICE, "total_count:%d, def_size:%d, last_size:%d", total_count, down_def_size, last_size);
	...
	if(file_fd != NULL)
	{
		...
		if(g_down_info->type == M2M_TCP)
		{
			...
			while((tcp_socket = connect_m2m_tcp_server(0, 0, g_m2m_server_ip, m2m_config.svr_port)) < 0)
			{
				...
				if(retry >= 5)
				{
					syslog(LOG_ERR, "Fail to connect download TCP, over retry.");
					close(file_fd);
					return 0;
				}
			...
			{
				syslog(LOG_NOTICE, "Having down:%d", recv_count);
				
				if(retry >= 3)		//times for require file
				{
					syslog(LOG_ERR, "Receive FILE_REQ_ACK fail, over retry.");
					close(tcp_socket);
					fclose(file_fd);
				...
				if(recv_count + 1 == total_count)
					m2m_tcp_send_file_req(FILE_REQ, ntohl(g_down_info->filelist[0].id), recv_count * down_def_size, last_size, pack_id);
				else
					m2m_tcp_send_file_req(FILE_REQ, ntohl(g_down_info->filelist[0].id), recv_count * down_def_size, down_def_size, pack_id);

				memset(recv_buf, 0, tcp_down_recv_size);
				
				recv_len = tcp_socket_recv(recv_buf, tcp_down_recv_size);
			...
			syslog(LOG_NOTICE, "download file:%s", g_down_info->filelist[0].md5);
			return 1;
			/*//if(strcmp(g_down_info->filelist[0].md5, upper(md5)) == 0)
			//{
				return 1;
			//}
			//else
			{
				syslog(LOG_ERR, "MD5 Error!");
				return 0;
			}*/
...
int send_download_ack(unsigned short cmd,unsigned int pack_id, unsigned char status)
{
    char hb_buf[1024] = {0};
    int hb_len = 0;
    M2M_PROTOCOL_HDR* hb = (M2M_PROTOCOL_HDR*)hb_buf;
            ...
            case DOWNLOAD_INFO:
				...
				if((!g_downloading) && ((pdu_len - sizeof(M2M_PROTOCOL_HDR)) == (g_down_info->filecount * sizeof(FILE_INFO) + 4)))
				{
					...
					if(g_down_info)
					{
						...
						if(download_process(pack_id))
						{
							send_download_ack(DOWNLOAD_INFO_ACK, pack_id, 0);
						...
						{
							send_download_ack(DOWNLOAD_INFO_ACK, pack_id, 1);
