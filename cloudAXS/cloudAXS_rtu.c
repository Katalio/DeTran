...
const char dest_ip[16] = "192.168.1.8";
char comma[] = ",";
char quote[] = "'";
static int rate = 0;
...
static void minimum_data(char *data)
{
	...
	//strcat(main_buff, nvram_get_int("account_id"));		
...
//	if(!nvram_match("sys_information_on_cbox", '0') 	\
									&& !nvram_match("net_information_on_cbox", '0')	\
									&& !nvram_match("data_usage_on_cbox", '0'))		//???
//	{
		minimum_data(data);		//最小包
//	}
//	else
//	{
		minimum_data(data);	
	...
	//	memset(s, 0, TMP_LEN);
		...
		strcat(data_usage_buff, comma);
		strcat(data_usage_buff, nvram_safe_get("active_vpns"));	
		strcat(data_usage_buff, comma);
