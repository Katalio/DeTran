...
{
    if (ctx->debug) {
        syslog(LOG_ERR, "ERROR %s", modbus_strerror(errno));
        if (context != NULL) {
            syslog(LOG_ERR, ": %s\n", context);
        } else {
            syslog(LOG_ERR, "\n");
...
int modbus_flush(modbus_t *ctx)
{
    ...
    if (rc != -1 && ctx->debug) {
        syslog(LOG_INFO,"%d bytes flushed\n", rc);
    }
    return rc;
}
...
/* BEGIN: modifed by zhangguocheng , del static */
...
*/
/* BEGIN: modifed by zhangguocheng , del */
//typedef enum {
...
*/
/* BEGIN: modified by zhangguocheng , del static */
...
/* END */
{
    ...
    if (ctx->debug) {
        if (msg_type == MSG_INDICATION) {
            syslog(LOG_INFO,"Waiting for a indication...\n");
        } else {
            syslog(LOG_INFO,"Waiting for a confirmation...\n");
        }
    }
#endif
    ...
    while (length_to_read != 0) {
        ...
        if (ctx->debug) {
	        ...
	        //	ptr = &str[i * 3];
//	        //   ptr = &str[i * 2];
			//    sprintf(ptr,"%02x ",(unsigned char )*(msg + msg_length + i));
	        //}
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    case _FC_READ_COILS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
            ...
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in read_bits\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_READ_DISCRETE_INPUTS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
            ...
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in read_input_bits\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_READ_HOLDING_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
            ...
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in read_registers\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_READ_INPUT_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
            ...
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in read_input_registers\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_WRITE_SINGLE_COIL:
        if (address >= mb_mapping->nb_bits) {
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in write_bit\n",
                        address);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_WRITE_SINGLE_REGISTER:
        if (address >= mb_mapping->nb_registers) {
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in write_register\n",
                        address);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_WRITE_MULTIPLE_COILS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_BITS < nb) {
            ...
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in write_bits\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_WRITE_MULTIPLE_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb) {
            ...
            if (ctx->debug) {
                syslog(LOG_ERR, "Illegal data address %0X in write_registers\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_READ_EXCEPTION_STATUS:
        if (ctx->debug) {
            syslog(LOG_ERR, "FIXME Not implemented\n");
        }
        errno = ENOPROTOOPT;
        return -1;
        break;
...
/* Reads the holding registers of remote device and put the data into an
   array */
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest,int type)
{
    int status;
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    if (nb > MODBUS_MAX_WRITE_BITS) {
        if (ctx->debug) {
            syslog(LOG_ERR, "ERROR Writing too many bits (%d > %d)\n",
                    nb, MODBUS_MAX_WRITE_BITS);
        }
        errno = EMBMDATA;
        return -1;
