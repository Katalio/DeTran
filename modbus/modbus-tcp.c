...
static int _modbus_tcp_init_win32(void)
{
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        syslog(LOG_ERR, "WSAStartup() returned error code %d\n",
                                       ...
                                       const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[0] != rsp[0] || req[1] != rsp[1]) {
        if (ctx->debug) {
            syslog(LOG_ERR, "Invalid TID received 0x%X (not 0x%X)\n",
...
int modbus_tcp_accept(modbus_t *ctx, int *socket)
{
    ...
    if (ctx->debug) {
        syslog(LOG_INFO,"The client connection from %s is accepted\n",
...
int modbus_tcp_pi_accept(modbus_t *ctx, int *socket)
{
    ...
    if (ctx->debug) {
        syslog(LOG_INFO,"The client connection is accepted.\n");
    }
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    if (sigaction(SIGPIPE, &sa, NULL) < 0) {
        ...
        syslog(LOG_ERR, "Coud not install SIGPIPE handler.\n");
        return NULL;
    }
#endif
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    if (ret_size == 0) {
        syslog(LOG_ERR, "The node string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        syslog(LOG_ERR, "The node string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size == 0) {
        syslog(LOG_ERR, "The service string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        syslog(LOG_ERR, "The service string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
