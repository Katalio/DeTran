                                           ...
                                           uint8_t *req)
{
	...
	if(addr == 0x0003)
		{
			 req[4] = (nb + 1) >> 8;
  			 req[5] = (nb + 1) & 0x00ff;  // 2:endflag byte and reserve byte 
	}
	else
		{
			 req[4] = nb >> 8;
                                ...
                                const int msg_length)
{
    ...
    if (crc_calculated == crc_received) {
        ...
        if (ctx->debug) {
            syslog(LOG_ERR, "ERROR CRC received %0X != CRC calculated %0X\n",
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    if (ctx_rtu->w_ser.fd == INVALID_HANDLE_VALUE) {
        syslog(LOG_ERR, "ERROR Can't open the device %s (%s)\n",
    ...
    if (!GetCommState(ctx_rtu->w_ser.fd, &ctx_rtu->old_dcb)) {
        syslog(LOG_ERR, "ERROR Error getting configuration (LastError %d)\n",
                (int)GetLastError());
    ...
    default:
        dcb.BaudRate = CBR_9600;
        syslog(LOG_INFO,"WARNING Unknown baud rate %d for %s (B9600 used)\n",
    ...
    if (!SetCommState(ctx_rtu->w_ser.fd, &dcb)) {
        syslog(LOG_ERR, "ERROR Error setting new configuration (LastError %d)\n",
                (int)GetLastError());
        CloseHandle(ctx_rtu->w_ser.fd);
        ctx_rtu->w_ser.fd = INVALID_HANDLE_VALUE;
        return -1;
...
#else
        if (ctx->debug) {
            syslog(LOG_ERR, "This function isn't supported on your platform\n");
        }
        errno = ENOTSUP;
        return -1;
#endif
...
void _modbus_rtu_close(modbus_t *ctx)
{
    ...
    if (!SetCommState(ctx_rtu->w_ser.fd, &ctx_rtu->old_dcb))
        syslog(LOG_ERR, "ERROR Couldn't revert to configuration (LastError %d)\n",
                (int)GetLastError());

    if (!CloseHandle(ctx_rtu->w_ser.fd))
        syslog(LOG_ERR, "ERROR Error while closing handle (LastError %d)\n",
                (int)GetLastError());
#else
                       ...
                       struct timeval *tv, int length_to_read)
{
    ...
    while ((s_rc = select(ctx->s+1, rfds, NULL, NULL, tv)) == -1) {
        if (errno == EINTR) {
            if (ctx->debug) {
                syslog(LOG_ERR, "A non blocked signal was caught\n");
            }
                         ...
                         int stop_bit)
{
    ...
    if (ret_size == 0) {
        syslog(LOG_ERR, "The device string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        syslog(LOG_ERR, "The device string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
