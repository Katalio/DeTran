...
{
	FILE	*fp = NULL;
	if (NULL == filename || NULL == md5 || NULL == (fp = fopen(filename, "rb")))
	{
		return (-1);
	}
	...
	fseek(fp, 0, SEEK_SET);
	if (fread(buf, filesize, 1, fp) <= 0)
	{
		fclose(fp);
		free(buf);
		return (-1);
	}
	md5_code_string(buf, filesize, md5);
...
{
         unsigned char MD5str[100] = "";
         unsigned char *input = MD5str;
         char *DEMD5str = NULL;
        
	printf("输入要转换的字符：\n");
        scanf("%s",MD5str);
        DEMD5str = MD5_CodeString(input);
